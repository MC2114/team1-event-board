...
describe("EventController", () => {
  it("forwards searchQuery to the service and renders success response", async () => {
    ...
    expect((res.render as unknown as jest.Mock).mock.calls[0][0]).toBe("events/index");
  ...
  it("renders partial results for HTMX success requests", async () => {
    const service = {
      listEvents: jest.fn().mockResolvedValue({ ok: true, value: [] as IEventRecord[] }),
    } as unknown as IEventService;
    const logger = {
      info: jest.fn(),
      warn: jest.fn(),
    ...
    expect((res.render as unknown as jest.Mock).mock.calls[0][0]).toBe("events/results");
  });

  it("renders partial error for HTMX failure requests", async () => {
    const service = {
      listEvents: jest.fn().mockResolvedValue({
        ok: false,
        value: { name: "InvalidInputError", message: "Invalid timeframe" },
    ...
    expect(res.status).toHaveBeenCalledWith(400);
    expect((res.render as unknown as jest.Mock).mock.calls[0][0]).toBe("partials/error");
  });
});
